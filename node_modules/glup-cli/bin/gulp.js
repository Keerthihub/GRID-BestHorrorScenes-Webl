...
function createApplication (name, dir) {
  ...
  // JavaScript
  var app = loadTemplate('js/gulpfile.js')
...
/*   app.locals.uses.push('express.json()')
 ...
 /*  app.locals.modules.CookieParser = 'cookie-parser'
  ...
  mkdir(dir, 'src')
  mkdir(dir, 'src/static')
  mkdir(dir, 'src/static/assets')
  mkdir(dir, 'src/static/assets/img')
  mkdir(dir, 'src/static/assets/fonts')
  mkdir(dir, 'src/static/js')
  mkdir(dir, 'src/static/stylesheets')

  // copy css templates
  switch (program.css) {
    case 'less':
      copyTemplateMulti('css', dir + '/src/static/stylesheets', '*.less')
      break
    case 'stylus':
      copyTemplateMulti('css', dir + '/src/static/stylesheets', '*.styl')
      break
    case 'compass':
      copyTemplateMulti('css', dir + '/src/static/stylesheets', '*.scss')
      break
    case 'sass':
      copyTemplateMulti('css', dir + '/src/static/stylesheets', '*.sass')
      break
    default:
      copyTemplateMulti('css', dir + '/src/static/stylesheets', '*.css')
      break
  }
 ...
 */
  if (program.view) {
    ...
    copyTemplate('js/index.html', path.join(dir, 'src/index.html'))
  }
...
/*   app.locals.localModules.usersRouter = './routes/users'
  ...
  // write files
  write(path.join(dir, 'gulpfile.js'), app.render())
...
function main () {
  ...
  emptyDirectory(destinationPath, function (empty) {
    if (empty || program.force) {
      ...
      confirm('destination is not empty, continue?\r\n目的地不为空，是否继续？ [y/N] ', function (ok) {
        if (ok) {
          process.stdin.destroy()
          createApplication(appName, destinationPath)
        } else {
          console.error('aborting 中止')
          exit(1)
        }
      })
    }
